    ...
    <div class="border-b border-gray-900/10 pb-12">
      ...
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
        ...
        <div class="sm:col-span-1"> 
          ...
          <div class="mt-2">
            <input type="text" name="permintaan_kategori" id="permintaan_kategori" class="block w-full rounded-md border-0 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6 p-2 bg-gray-200" placeholder="masukkan kategori pengajuan aset" required readonly>
          </div>
        </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Endpoint untuk mendapatkan data aset
  ...
  .then(data => {
    ...
    document.getElementById('permintaan_kategori').value = 'Pengajuan Service/Ganti Aset'; // Set kategori otomatis
  })
  .catch(error => {
  ...
  document.getElementById('pengajuanServiceGantiForm').addEventListener('submit', function(event) {
    ...
    const data = {
      ...
      permintaan_status: formData.get('permintaan_status'),  // Add missing field
      permintaan_tujuan: formData.get('permintaan_tujuan'),
      permintaan_kuantitas: formData.get('permintaan_kuantitas'),  // Add missing field
    };

    // Mengirimkan data ke API untuk membuat request
    ...
    .then(result => {
        ...
        // Anda bisa menambahkan handling sukses, seperti menampilkan pesan atau redirect
    })
    .catch(error => {
        console.error('Error submitting request:', error);
        // Anda bisa menambahkan handling error, seperti menampilkan pesan error
    });
});
});
